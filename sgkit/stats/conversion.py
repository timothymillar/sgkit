...
def convert_probability_to_call(
    ...
    allele_unique
        ...
        If the variable cannot be automatically computed, then all variants are assumed to
        be biallelic.
    merge
        If True (the default), merge the input dataset and the computed
        output variables into a single dataset, otherwise return only
    ...
    if allele_unique not in ds:
        if variables.variant_allele in ds:
            ...
            max_alleles = ds.dims.get("alleles") or u_alleles.max().compute()
        else:
        ...
        u_alleles = da.array(ds[allele_unique].data)[:, None]
        max_alleles = ds.dims.get("alleles") or u_alleles.max().compute()
